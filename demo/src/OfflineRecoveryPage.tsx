...
export function OfflineRecoveryPage({ onReconnect, onBack }: { onReconnect: () => void; onBack: () => void }) {
  ...
  const [busy, setBusy] = useState(false); const [deleteTarget, setDeleteTarget] = useState<{ kind: 'outbox' | 'draft' | 'taskDraft'; key: string } | null>(null);
  ...
  const load = useCallback(async (kind: 'initial' | 'content' | 'check' | 'more', cursor?: string) => {
    ...
    try {
      ...
      setSnapshot((current) => kind === 'check' && current ? { ...current, identity: data.identity, outbox: data.outbox, taskDrafts: data.taskDrafts } : kind === 'more' && current ? { ...data, drafts: [...current.drafts, ...data.drafts] } : data);
  ...
  async function validateDownloadIdentity() {
    const expected = revision.current;
    if (!active.current || !expected || closedRef.current) throw new Error('本机身份已改变，下载已取消。');
    try {
      const current = await readOfflineSnapshot(expected);
      if (!active.current || closedRef.current || revision.current !== expected || !current || current.identity.revision !== expected) { if (active.current) closeIdentity(); throw new Error('本机身份已改变，下载已取消。'); }
    } catch (cause) { if (active.current && cause instanceof APIError && cause.code === 'LOCAL_IDENTITY_CHANGED') closeIdentity(); throw cause; }
  }
  ...
  async function remove() {
    ...
    try {
      ...
      if (!active.current || closedRef.current) return;
      setDeleteTarget(null); setNotice('本机条目已删除；服务器已经收到的消息与任务不受影响。'); await load('content');
    } catch (cause) {
      if (cause instanceof APIError && cause.code === 'LOCAL_IDENTITY_CHANGED') closeIdentity();
      else if (active.current && !closedRef.current) setError(cause instanceof Error ? cause.message : '删除失败，请重试。');
    } finally { if (active.current) setBusy(false); }
  }
  return <div className="offline-recovery"><header className="offline-recovery-header"><Brand /><button className="secondary-button" onClick={onBack}><ArrowLeft size={16} />返回连接页</button></header><main><header className="workspace-page-header"><div><h1><WifiOff size={24} />离线本机内容</h1><p>账号状态尚未验证。聊天待发可在验证身份后继续投递；任务草稿须本人联网复核并确认提交。</p></div><button className="primary-button" onClick={onReconnect}><RefreshCw size={16} />重新连接</button></header><div className="queue-explanation"><p>这里只显示此浏览器当前本机身份的待发消息、聊天草稿和任务草稿，不包含服务器聊天历史；此页面不会发送或提交任何内容。</p><p>已退出并选择保留的旧账号内容，需要重新登录对应账号后才能访问。</p></div>{error && <p className="form-error" role="alert">{error}</p>}{notice && <p className="success-note" role="status">{notice}</p>}{loading && !snapshot && <p role="status">正在读取本机内容…</p>}{snapshot && !closed && <><p className="offline-identity">本机记录：<strong>{snapshot.identity.user.nickname}</strong> @{snapshot.identity.user.username}<span>账号状态未验证</span></p><section><h2>本机待发</h2>{snapshot.outbox.length ? <ul className="outbox-list">{snapshot.outbox.map((item) => <li className="outbox-card" key={item.key}><header><strong>{item.conversationTitle}</strong><span className="queue-state">上次状态：{item.state === 'queued' ? '等待投递' : item.state === 'sending' ? '等待服务器确认' : '发送失败'}</span></header><p className="outbox-message">{item.payload.text || '（没有正文）'}</p><LocalAttachmentList files={item.files} allowDownload beforeDownload={validateDownloadIdentity} />{item.error && <p className="field-error">{item.error}</p>}<div className="outbox-meta">保存于 {new Date(item.createdAt).toLocaleString('zh-CN')}</div><footer><button className="secondary-button" disabled={busy} onClick={() => void copy(item.payload.text)}><Copy size={15} />复制正文</button><button className="text-button danger-text" disabled={busy} onClick={() => setDeleteTarget({ kind: 'outbox', key: item.key })}><Trash2 size={15} />删除本机条目</button></footer></li>)}</ul> : <p className="section-help">这个本机身份没有待发消息。</p>}</section><section><h2>本机草稿</h2>{snapshot.drafts.length ? <ul className="outbox-list">{snapshot.drafts.map((item) => <li className="outbox-card" key={item.key}><header><strong>{snapshot.outbox.find((pending) => pending.conversationId === item.conversationId)?.conversationTitle || '会话草稿'}</strong><time>{new Date(item.updatedAt).toLocaleString('zh-CN')}</time></header><p className="outbox-message">{item.text}</p><LocalAttachmentList files={item.files || []} allowDownload beforeDownload={validateDownloadIdentity} /><footer><button className="secondary-button" disabled={busy} onClick={() => void copy(item.text)}><Copy size={15} />复制草稿</button><button className="text-button danger-text" disabled={busy} onClick={() => setDeleteTarget({ kind: 'draft', key: item.key })}><Trash2 size={15} />删除本机草稿</button></footer></li>)}</ul> : <p className="section-help">这个本机身份没有已保存的草稿。</p>}{snapshot.nextDraftCursor && <button className="load-more-button" disabled={loading} onClick={() => void load('more', snapshot.nextDraftCursor!)}>{loading ? '正在加载…' : '加载更多草稿'}</button>}</section><section className="offline-task-drafts"><h2>本机任务草稿</h2><p className="warning-note">这些是当前账号尚未验证的本机输入，权限和任务内容可能已改变。恢复网络不会自动提交；7 天过期后需本人确认建立新草稿。</p>{snapshot.taskDrafts?.filter((item) => item.userId === snapshot.identity.user.id).length ? <ul className="outbox-list">{snapshot.taskDrafts.filter((item) => item.userId === snapshot.identity.user.id).map((item) => <li className="outbox-card" key={item.id}><header><strong>{item.payload.title || '待办修改草稿'}</strong><span>{item.kind === 'create' ? '创建草稿' : '修改草稿'} · {item.expiresAt <= Date.now() ? '已过期，仅可保留或复制输入' : '尚未提交'}</span></header><details><summary>查看本机任务输入</summary><p className="outbox-message">{item.payload.description || '未填写描述。'}</p><p>时区：{item.payload.dueTimezone || '未填写'} · 截止：{item.payload.dueOn || '无'}</p></details><time>保存于 {new Date(item.updatedAt).toLocaleString('zh-CN')}</time><footer><button className="secondary-button" disabled={busy} onClick={() => void copy([item.payload.title || '', item.payload.description || ''].filter(Boolean).join('\n\n'))}><Copy size={15} />复制任务草稿</button><button className="text-button danger-text" disabled={busy} onClick={() => setDeleteTarget({ kind: 'taskDraft', key: item.id })}><Trash2 size={15} />删除本机任务草稿</button></footer></li>)}</ul> : <p className="section-help">这个本机身份没有已保存的任务草稿。</p>}</section></>}{!loading && !snapshot && !closed && !error && <EmptyState title="没有可展示的本机内容" description="当前没有活跃的本机身份。请重新连接并登录后查看对应账号的内容。" />}{!loading && !snapshot && !closed && error && <button className="secondary-button" onClick={() => void load(initialized.current ? 'content' : 'initial')}>重新读取本机内容</button>}</main><Modal open={!!deleteTarget} title="删除本机内容" dismissible={!busy} onClose={() => setDeleteTarget(null)}><p>这会删除当前条目在本机的副本。不会撤回或删除服务器已经收到的消息或任务，删除后无法从本机恢复。</p>{deleteTarget && error && <p className="form-error" role="alert">{error}</p>}<button className="primary-button" disabled={busy} onClick={() => void remove()}>{busy ? '正在删除…' : '确认删除本机条目'}</button></Modal></div>;
}
