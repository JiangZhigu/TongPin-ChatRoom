...
export function App() {
  const pathname = demoPath().split('?')[0];
...
function canonicalizeAuthenticatedRegistration(admin: boolean) {
  if (!admin && demoPath().split('?')[0].replace(/\/+$/, '') === '/register') {
...
function AccountApplication({ admin }: { admin: boolean }) {
  ...
  if (!bootstrap.user) return <><AuthPage bootstrap={bootstrap} admin={admin} initialMode={demoPath().split('?')[0].replace(/\/+$/, '') === '/register' ? 'register' : 'login'} onAuthenticated={authenticated} />{invitationToken && (invitationOpen ? <GroupInviteEntry key={invitationToken} token={invitationToken} userId={null} onClose={closeInvitation} onSignIn={() => setInvitationOpen(false)} onOpenGroup={async () => {}} /> : <button className="invitation-resume-button secondary-button" onClick={() => setInvitationOpen(true)}>继续查看群邀请</button>)}</>;
