...
const DATABASE = 'tongpin-demo-local-v1';
const VERSION = 2;
...
function announceLocal(kind: 'identity' | 'content', userId?: string) {
  ...
  if (typeof BroadcastChannel !== 'undefined') {
    const channel = new BroadcastChannel('tongpin-demo-state-v1');
...
export function subscribeOfflineChanges(listener: (kind: 'identity' | 'content' | 'check') => void): () => void {
  const channel = typeof BroadcastChannel !== 'undefined' ? new BroadcastChannel('tongpin-demo-state-v1') : null;
...
export async function withDeliveryLock(userId: string, owner: string, signal: AbortSignal, work: (stillOwner: () => boolean) => Promise<void>): Promise<void> {
  ...
  if (navigator.locks?.request) {
    try {
      await navigator.locks.request('tongpin-demo-delivery:' + userId, { ifAvailable: true }, async (lock) => {
