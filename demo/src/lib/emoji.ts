...
export function recentEmoji(userId: string): string[] {
  try { const value: unknown = JSON.parse(localStorage.getItem('tongpin-demo-emoji-recent:' + userId) || '[]'); return Array.isArray(value) ? value.filter((item): item is string => typeof item === 'string' && item.length <= 100).slice(0, 24) : []; } catch { return []; }
}
export function rememberEmoji(userId: string, sequence: string): void {
  try { localStorage.setItem('tongpin-demo-emoji-recent:' + userId, JSON.stringify([sequence, ...recentEmoji(userId).filter((item) => item !== sequence)].slice(0, 24))); } catch { /* Recent choices are optional; blocked storage does not prevent insertion. */ }
}
