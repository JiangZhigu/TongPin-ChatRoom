...
function currentLocation() { return demoPath().split('?')[0] + demoSearch(); }
function Route({ url }: { url: string }) {
  const parsed = new URL(url, 'http://tongpin.demo'); const path = parsed.pathname.replace(/\/$/, ''); const params = parsed.searchParams;
...
export function AdminShell({ user, onSignOut, signOutBusy, signOutError }: { user: UserView; onSignOut: () => void; signOutBusy: boolean; signOutError: string }) {
  ...
  const navigate = useCallback((next: string) => { const parsed = new URL(next, 'http://tongpin.demo'); if (parsed.origin !== 'http://tongpin.demo' || !(parsed.pathname === '/admin' || parsed.pathname.startsWith('/admin/'))) return; window.history.pushState({}, '', parsed.pathname + parsed.search); setRequest(null); setMenuOpen(false); setUrl(currentLocation()); }, []);
