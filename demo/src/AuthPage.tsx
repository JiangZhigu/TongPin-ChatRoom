...
export function AuthPage({ bootstrap, admin, initialMode = 'login', onAuthenticated }: { bootstrap: BootstrapView; admin: boolean; initialMode?: Mode; onAuthenticated: (result: AuthResult) => void }) {
  ...
  function switchMode(next: Mode) { if (next === mode) return; clearCaptcha(); setLoginCaptchaRequired(false); setMode(next); setPassword(''); setConfirmation(''); setAnswer(''); setSecondFactor(''); setRecoveryCode(''); setError(''); setNotice(''); setFieldErrors({}); }
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (cause instanceof APIError) { setError(cause.message); setFieldErrors(cause.fieldErrors || {}); if (cause.code === 'SECOND_FACTOR_REQUIRED' || cause.code === 'SECOND_FACTOR_INVALID') setShowFactor(true); if (cause.retryAfterMs) setRetryAt(Date.now() + cause.retryAfterMs); }
