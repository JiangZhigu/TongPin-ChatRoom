...
class PolicyService:
    ...
    def terms(self):
        ...
        return {
            ...
            "text": f"同频服务由站点运营者管理。全站超级管理员可以审阅私聊、群聊和附件，敏感访问会记录审计；本服务不提供端到端加密。未删除消息默认持续保存；撤回或管理删除的内容立即对普通用户隐藏，当前管理保留期为{policy['deleted_content_days']}天；审计保留{policy['audit_days']}天。注销有{policy['deletion_cooling_days']}天冷静期，注销后共享消息按保留规则保存并显示已注销身份。注册后无需保存恢复码；忘记密码时请联系站点管理员核实身份，由管理员协助重设密码。文件白名单不能保证文件安全，请谨慎下载和打开。"
