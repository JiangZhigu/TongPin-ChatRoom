...
当前：**M1–M9、完整SA01–SA14及V3完整P0/P1的阶段实现、验收及交付准备已完成。2026-09-12追加开放注册与连续5次密码错误才要求验证码，本机定向、真实注册/登录流程和独立审查通过。新三平台CI各63项后端通过，前端各444通过/1项旧登录验证码测试失败；该测试已修正、所在文件2项及类型检查通过，等待CI复跑。用户已允许最终新同等级Astra使用agent-browser驱动Edge进行完整体验，随后修复复测并正式交付。** FE-1持续执行，无逐阶段确认点。重大事项见[待确认清单](PENDING_CONFIRMATIONS.zh-CN.md)。
...
| 注册/登录追加 | 本机定向/浏览器/审查通过，CI待复跑 | 68项不同后端用例有通过证据，35项UI及类型检查通过；真实注册/恢复码确认/密码登录/验证码阈值与重置通过；首轮新CI各63项后端通过，前端444通过/1项旧测试失败，该文件2项修正后通过；见[批次记录](REGISTRATION_LOGIN_DELIVERY.zh-CN.md) |
| UX-POST | 工具已确认，交付前执行 | 本批注册/登录追加检查完成后另起同等级Astra，用agent-browser驱动Edge完整体验聊天室、待办与超管后台，修复复测并更新交付物 |
...
personal-subagents持续启用，standard；含历史9个不同子智能体，注册/登录追加使用新的Git和UI角色，独立审查及最终新Astra按需保留。主智能体负责核心逻辑、整合、记录和真实环境检查。累计预算13个不同子智能体、最多3个并行、26轮浏览器；当前实际20轮（历史19轮加本批注册验证1轮），另预留3平台CI及最终Edge体验/修复。历史实例不再存活所需替补及新增认证风险为预算更新依据，未重置累计计数；准确派发状态以共享索引为准。
