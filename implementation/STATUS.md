...
当前：**M1–M9、完整SA01–SA14及V3完整P0/P1的阶段实现、验收及交付准备已完成。2026-09-12追加开放注册与连续5次密码错误才要求验证码，本机定向检查及真实注册/登录页面流程通过，进入独立审查和新三平台CI。用户已允许最终新同等级Astra使用agent-browser驱动Edge进行完整体验，随后修复复测并正式交付。** FE-1持续执行，无逐阶段确认点。重大事项见[待确认清单](PENDING_CONFIRMATIONS.zh-CN.md)。
...
| 注册/登录追加 | 本机定向及浏览器通过 | 68项不同后端用例有通过证据，35项UI及类型检查通过；真实注册/恢复码确认/密码登录/验证码阈值与重置通过；独立审查和新CI待完成 |
