...
当前：**M1–M4 已完成本机定向验证、提交与本批审查；M3/M4的三项修复在64ff561复核关闭。M5群聊实现及本机定向验证完成，等待冻结提交与独立审查。V3的P0+P1全范围已获确认，重要权限/保留规则及最终体验工具选择分别等待已发出的回复。** FE-1持续执行，无逐阶段确认点。非常重要的新事项只暂停相关动作，写入[待确认清单](PENDING_CONFIRMATIONS.zh-CN.md)。
...
| M5 | 本机验证完成，待审查 | 建群、邀请审核、角色、since_join权限、禁言/踢出/转让/解散；13项群后端、67项相关界面、13项客户端及真实三账号群流程，发现的草稿/权限窗口/提示与入口问题已定向修复复验；见 [M5交付](M5_DELIVERY.zh-CN.md) |
