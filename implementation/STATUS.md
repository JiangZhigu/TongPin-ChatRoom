...
当前：**M1/M2 已完成本机定向验证、提交与本批审查；M3/M4 好友私聊和离线同步已实现并完成定向测试及双浏览器实测，正在冻结审查。V3 的 P0+P1 全范围已获确认并入本次交付，重要权限/保留规则待一次回复。** FE-1 持续执行，无逐阶段确认点。非常重要的新事项只暂停相关动作，写入[待确认清单](PENDING_CONFIRMATIONS.zh-CN.md)。
