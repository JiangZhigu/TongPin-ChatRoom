...
当前：**M1–M7、完整SA01–SA14及V3完整P0/P1已实现并完成阶段检查/审查修复。M8首次全量暴露的问题已定向修复；600秒100连接负载6,005/6,005成功，p95 217.14ms，所有接收者补收/附件字节/数据库一致性通过。M9三系统/Docker脚本、预检/打包及CI已实现，14项部署检查和Windows真实浏览器冒烟通过，正在冻结新全量、审查、三平台CI及真实发布升级演练。最终Astra browser-use工具选择仍待回复。** FE-1持续执行，无逐阶段确认点。重大事项见[待确认清单](PENDING_CONFIRMATIONS.zh-CN.md)。
...
| M8 | 首次全量问题已修复，600秒负载通过，最终全量待运行 | [联合回归/负载](M8_DELIVERY.zh-CN.md)、[A–G/SA逐项映射](M8_ACCEPTANCE.zh-CN.md)；真实SQLite BUSY/FULL故障恢复2项通过 |
| M9 交付准备 | 脚本/本机定向检查通过，CI/发布演练进行中 | 14项部署专属检查、真实WS/浏览器通过；安装/配置/用户/管理员/运维文档齐全；[M9记录](M9_DELIVERY.zh-CN.md) |
...
personal-subagents持续启用，standard；已用9个不同子智能体（含M8专属Terra回归角色），第10个保留给最终新Astra体验。主智能体负责核心逻辑、整合、记录和真实环境检查。累计预算10个不同子智能体、最多3个并行、18轮浏览器；当前登记14轮，含阶段12轮和M9脚本调试/通过的2轮。缺少浏览器二进制的准备失败没有启动页面，复用原预留回合。
