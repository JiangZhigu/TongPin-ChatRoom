...
更新：2026-09-12。依据用户原始A01–G09及SA01–SA14，保留原设计文件不回写勾选。本表的“已覆盖”指对应自动化/阶段浏览器证据；最终冻结全量和三平台CI已通过，UX-POST及实际设备/部署边界明确标注。阶段记录包含具体版本、测试ID、真实场景和局限，不能把测试文件存在当作运行通过。
...
| G06 | 迁移/升级快照/代码回滚 | 基础迁移与部署单元通过；M9真实双目录安装/切换/回滚保留消息、待办及附件字节；相同提交/schema，非历史版本迁移声称 |
...
| G09 | 三系统及Docker部署/环境/回退 | M9入口/脚本和模板已实现；三平台CI与非root只读Docker内HTTP/WS实测通过；实际目标主机服务托管/TLS未实施 |
...
| SA14 运维任务 | S3真实异步导出/备份/预检/隔离演练；test_admin_operations/restore租约/取消/失败恢复；M9同schema服务版本切换/回滚及3份逐文件快照通过 |

上述管理模块已经过各批冻结增量审查和缺陷关闭，共享Runtime变化已由新冻结212项后端全量回归承接。没有把普通群角色当作站点管理员，个人V3待办也没有普通全站检索入口。
