...
`manage init-admin` 要求服务已停止，取数据目录单实例锁，全程中文提示，仅交互输入账号、8–128字符密码和确认密码，不绑定验证器、不生成恢复码。账号格式、重名、密码强度或确认密码有误时会提示并允许重试，按 Ctrl+C 可取消。显示名默认使用账号，可在登录后修改。密码不经过命令参数。已有超管时提示无需重复初始化。数据被服务占用或无法保存时会提示原因并安全结束，不显示异常堆栈。
