...
describe('M7-UI workspace rich drafts and location', () => {
  it('restores ID-only reply and mention metadata, clears submitted IDs and preserves new reply during queue commit', async () => {
    ...
    fireEvent.click(within(screen.getByText('原消息 B').closest('li')!).getByRole('button', { name: '消息菜单' })); fireEvent.click(screen.getByRole('menuitem', { name: '引用回复' })); fireEvent.change(screen.getByLabelText('消息内容'), { target: { value: '后续正文' } });
...
describe('M7-FIX-UI workspace mutation wiring', () => {
  it('passes captured core tokens through message and bookmark callbacks without assembling stale bookmark bodies', async () => {
    const token = Object.freeze({ generation: 2, contentRevision: 8 }); chat.beginUpdate.mockReturnValue(token); const original = { ...message(), capabilities: { canInteract: true, canRecall: true, canModerate: false } }; chat.histories['dm-a'] = [original]; const pending = deferred<{ ok: boolean; status: number; json: () => Promise<unknown> }>();
    const fetchMock = vi.mocked(fetch); fetchMock.mockImplementation((url) => String(url).endsWith('/bookmark') ? pending.promise as Promise<Response> : response({ admin: null }) as Promise<Response>); showWorkspace(); await openConversation(); fireEvent.click(within(screen.getByLabelText('消息记录')).getByRole('button', { name: '消息菜单' })); fireEvent.click(screen.getByRole('menuitem', { name: '收藏' })); const tombstone = { ...original, status: 'recalled' as const, text: '', reactions: [] }; act(() => publish({ messages: [tombstone] })); await act(async () => pending.resolve({ ok: true, status: 200, json: async () => ({ data: { bookmarked: true } }) })); expect(chat.beginUpdate).toHaveBeenCalledOnce(); const bookmarkRequest = fetchMock.mock.calls.findIndex(([url]) => String(url).endsWith('/bookmark')); expect(bookmarkRequest).toBeGreaterThanOrEqual(0); expect(chat.beginUpdate.mock.invocationCallOrder[0]).toBeLessThan(fetchMock.mock.invocationCallOrder[bookmarkRequest]); expect(chat.bookmark).toHaveBeenCalledExactlyOnceWith(original.id, true, token); expect(chat.apply).not.toHaveBeenCalled(); expect(screen.getByText('这条消息已撤回')).toBeInTheDocument();
    act(() => publish({ messages: [original] })); fetchMock.mockImplementationOnce(() => response({ message: original }) as Promise<Response>); fireEvent.click(screen.getByRole('button', { name: '消息菜单' })); fireEvent.click(screen.getByRole('menuitem', { name: '撤回' })); fireEvent.click(screen.getByRole('button', { name: '确认' })); await waitFor(() => expect(chat.apply).toHaveBeenCalledExactlyOnceWith(original, token)); expect(chat.beginUpdate).toHaveBeenCalledTimes(2);
  });
});
...
describe('V3 source preview authority repair', () => {
  ...
  async function opening() { enableTasks(); chat.histories['dm-a'] = [message('source-message', '1', 'CAPTURED-OLD-SOURCE')]; showWorkspace(); await openConversation(); fireEvent.click(screen.getByRole('button', { name: '消息菜单' })); fireEvent.click(screen.getByRole('menuitem', { name: '转为待办' })); return screen.findByRole('dialog', { name: '从消息创建待办' }); }
