...
export function ChatWorkspace({ user, onUserChange, onSignedOut, invitationToken, onInvitationDismiss }: { user: UserView; onUserChange: (user: UserView) => void; onSignedOut: () => void; invitationToken?: string | null; onInvitationDismiss?: () => void }) {
  ...
  }, [client, section, selectedId, state.selectedId, state.messages, state.conversations, draftLoading, detailsOpen, createGroupOpen, joinGroupOpen, invitationToken, logoutPrompt, state.historyAfter, searchOpen, reportsOpen]);
  ...
  else if (section === 'notifications') content = <NotificationsPage items={state.notifications} hasMore={!!state.nextNotifications} onLoadMore={() => client.loadMoreNotifications()} onRefresh={() => client.refresh()} onOpenMessage={jumpToMessage} onOpenReports={() => setReportsOpen(true)} onOpenRequests={() => void navigate('contacts', 'requests')} onOpenGroup={(id) => openGroup(id, true)} />;
