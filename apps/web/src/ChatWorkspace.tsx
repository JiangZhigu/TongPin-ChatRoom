...
import { ArrowDown, Clock3, RefreshCw, Settings, UserRound, Wifi, WifiOff, X } from 'lucide-react';
...
import { blockedOutboxLabel, canRetryOutbox, failedOutboxGuidance, failedOutboxHeads, isWaitingOnFailedHead } from './lib/outbox-presentation';
...
export function ChatWorkspace({ user, onUserChange, onSignedOut, invitationToken, onInvitationDismiss }: { user: UserView; onUserChange: (user: UserView) => void; onSignedOut: () => void; invitationToken?: string | null; onInvitationDismiss?: () => void }) {
  ...
  const [accountPanel, setAccountPanel] = useState<'profile' | 'settings' | null>(null); const [accountBusy, setAccountBusy] = useState(false);
  ...
  async function openAccountPanel(next: 'profile' | 'settings') {
    const generation = ++selectionGeneration.current; capturePosition(); client.typing?.(false);
    try {
      await saveCurrentDraft();
      if (!mounted.current || generation !== selectionGeneration.current) return;
      setAccountBusy(false); setAccountPanel(next);
    } catch (cause) { if (mounted.current && generation === selectionGeneration.current) setError(`草稿保存失败，暂未打开设置。${describeError(cause)}`); }
  }
  function closeAccountPanel() {
    setAccountPanel(null); setAccountBusy(false);
    if (selectedRef.current) restorePosition.current = { id: selectedRef.current, position: positions.current.get(selectedRef.current) };
    setJumpVersion((value) => value + 1);
  }
  async function navigate(next: MainSection, tab?: ContactsTab) {
    if (next === 'settings') { await openAccountPanel('settings'); return; }
    if (next === section && !(tab && next === 'contacts')) return;
    if (next === 'tasks') { taskObscured.current = true; capturePosition(); client.typing?.(false); }
    const generation = ++selectionGeneration.current;
    try { await saveCurrentDraft(); if (!mounted.current || generation !== selectionGeneration.current) return; if (tab) setContactsTab(tab); if (next === 'messages' && selectedRef.current) restorePosition.current = { id: selectedRef.current, position: positions.current.get(selectedRef.current) }; setSection(next); setAccountPanel(null); setAccountBusy(false); taskOpeningGeneration.current++; setTaskPanel(null); setTaskOpening(false); taskObscured.current = false; setSearchOpen(false); setReportsOpen(false); setDetailsOpen(false); client.typing?.(false); }
  ...
  async function copyToDraft(item: QueuedMessage) {
    ...
    setNotice(item.state === 'failed' ? '已复制到编辑器，原失败项仍在本机待发中。请返回本机待发，停止原失败项的本机重试，让后续消息继续；确认原消息状态后再发送复制的内容。' : '已复制到编辑器，原待发项仍保留。再次发送会创建一条新消息，请先确认原消息状态。'); input.current?.focus();
  }
  ...
  else if (section === 'notifications') content = <NotificationsPage subscribeGroupEvents={client.subscribeTaskEvents} actorContext={user.id} items={state.notifications} hasMore={!!state.nextNotifications} onLoadMore={() => client.loadMoreNotifications()} onRefresh={() => client.refresh()} onOpenMessage={jumpToMessage} onOpenTask={(taskId) => openTaskPanel({ kind: 'detail', taskId })} onOpenReports={() => setReportsOpen(true)} onOpenRequests={() => void navigate('contacts', 'requests')} onOpenGroup={(id) => openGroup(id, true)} />;
