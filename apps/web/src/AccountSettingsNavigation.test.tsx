...
describe('settings grouped navigation', () => {
  ...
  it('keeps navigation locked while administrator verification is running until invitation acceptance completes', async () => {
    ...
    request.mockImplementation((path, options) => {
      ...
      if (path === '/api/v1/account/admin-enrollment/start') return Promise.resolve({ enrollmentId: 'test-enrollment', expiresAt: Date.now() + 60000 });
      if (path === '/api/v1/account/admin-enrollment/finish') return Promise.resolve({ user: { ...user, siteRole: 'super_admin' } });
      return original(path, options);
    });
    render(<AccountSettings embedded user={user} onUserChange={vi.fn()} onSignedOut={vi.fn()} onBusyChange={busy} />);
    select('管理权限邀请与绑定');
    fireEvent.change(await within(screen.getByRole('tabpanel', { name: '管理权限邀请与绑定' })).findByLabelText('当前密码'), { target: { value: 'isolated test password' } });
    fireEvent.click(screen.getByRole('button', { name: '验证密码并接受邀请' }));
