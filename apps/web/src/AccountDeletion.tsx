...
export function AccountDeletion({ user, onClose, onDeleted, onBeforeDelete, onDeleteFailed, onPreserveAndSignOut, onGetLocalSummary }: { user: UserView; onClose: () => void; onDeleted: (choice: 'keep' | 'delete') => Promise<void>; onBeforeDelete?: () => Promise<void>; onDeleteFailed?: () => void; onPreserveAndSignOut?: () => Promise<void>; onGetLocalSummary?: () => Promise<{ pending: number; drafts: number; taskDrafts: number }> }) {
  ...
  const [password, setPassword] = useState(''); const [confirmation, setConfirmation] = useState(''); const [choice, setChoice] = useState<'' | 'keep' | 'delete'>(''); const [deleted, setDeleted] = useState(false); const confirmedDeleted = useRef(false);
    ...
    try {
      if (!confirmedDeleted.current) {
        if (!submission.current) {
          ...
          const { reauthToken } = await api<{ reauthToken: string }>('/api/v1/auth/reauth', { method: 'POST', body: { password, action: 'account.delete' } });
        ...
        confirmedDeleted.current = true; submission.current = null; setDeleted(true); setUncertain(false); setPassword('');
      }
