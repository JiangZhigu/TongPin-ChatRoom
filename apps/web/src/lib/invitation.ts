...
export function parseInvitation(value: string): string | null {
  ...
  try { const url = new URL(trimmed, window.location.origin); const token = new URLSearchParams(url.hash.slice(1)).get('invite'); return url.origin === window.location.origin && token && tokenPattern.test(token) ? token : null; }
  catch { return null; }
}
...
export function invitationLink(token: string): string { if (!tokenPattern.test(token)) throw new Error('邀请链接无效。'); return window.location.origin + '/#invite=' + token; }
