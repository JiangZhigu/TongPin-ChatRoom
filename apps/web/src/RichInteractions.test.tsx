...
describe('M7-UI rich interactions', () => {
  it('hides actions until opened and omits legacy reactions and emoji controls', () => {
    render(<MessageActions onBeginUpdate={() => defaultUpdateToken} onBookmarked={vi.fn()} message={{ ...message, reactions: [{ key: '1F602', count: 1, mine: true }] }} userId={user.id} onUpdated={vi.fn()} onReply={vi.fn()} onCreateTask={vi.fn()} />);
    expect(screen.queryByRole('menu')).not.toBeInTheDocument();
    expect(screen.queryByText('1F602')).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: '消息菜单' }));
    expect(screen.getByRole('menuitem', { name: '转为待办' })).toBeInTheDocument();
    expect(screen.queryByText('添加反应')).not.toBeInTheDocument();
    expect(screen.queryByText('👍')).not.toBeInTheDocument();
    expect(request).not.toHaveBeenCalled();
  });
  it('supports keyboard navigation, escape focus return and outside dismissal', () => {
    render(<MessageActions onBeginUpdate={() => defaultUpdateToken} onBookmarked={vi.fn()} message={message} userId={user.id} onUpdated={vi.fn()} onReply={vi.fn()} />);
    const toggle = screen.getByRole('button', { name: '消息菜单' });
...
describe('M7-FIX-UI message update tokens', () => {
  it.each(['recall', 'moderate'] as const)('captures the original token before %s dispatch and carries it after a tombstone rerender', async (action) => {
    ...
    { fireEvent.click(screen.getByRole('button', { name: '消息菜单' })); fireEvent.click(screen.getByRole('menuitem', { name: action === 'recall' ? '撤回' : '管理删除' })); if (action === 'moderate') fireEvent.change(screen.getByLabelText('管理删除原因'), { target: { value: '管理原因' } }); fireEvent.click(screen.getByRole('button', { name: '确认' })); }
