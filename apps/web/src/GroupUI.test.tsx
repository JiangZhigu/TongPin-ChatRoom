...
describe('M5-UI group creation and management', () => {
  ...
  it('scopes reauthentication and ownership transfer to exact target period without optimistic role changes', async () => {
    ...
    const dialog = screen.getByRole('dialog', { name: '转让群主 · 再次验证' }); fireEvent.change(within(dialog).getByLabelText('当前密码'), { target: { value: 'test-only-password' } }); fireEvent.change(within(dialog).getByLabelText(/动态码/), { target: { value: '123456' } }); fireEvent.click(within(dialog).getByRole('checkbox')); fireEvent.click(within(dialog).getByRole('button', { name: '确认并继续' }));
    await screen.findByText('转让邀请已发出，受让者接受之前，你仍是群主。'); expect(updates[0]).toEqual({ url: '/api/v1/auth/reauth', body: { password: 'test-only-password', secondFactor: '123456', action: 'group_transfer:group-1' } }); expect(updates[1]).toMatchObject({ url: '/api/v1/groups/group-1/transfers', body: { expectedVersion: 7, targetUserId: 'friend', targetPeriodId: 'friend-current-period', reauthToken: 'scoped-reauth-token' } }); expect(current.conversation.role).toBe('owner');
  });
