...
describe('S3 command parameters and actual receipts', () => {
  ...
  it('distinguishes accepted task from a completed command receipt', async () => {
    ...
    render(<AdminActionDialog request={{ action: 'backup.create', targetIds: ['instance'], labels: ['实例'], parameters: {} }} onClose={vi.fn()} onChanged={vi.fn()} />); fireEvent.change(screen.getByLabelText('操作理由'), { target: { value: '执行备份' } }); fireEvent.click(screen.getByRole('button', { name: '预览目标与影响' })); await screen.findByRole('heading', { name: '服务端操作预览' }); fireEvent.change(screen.getByLabelText('管理员当前密码'), { target: { value: 'secret' } }); fireEvent.change(screen.getByLabelText('动态码或第二因素恢复码'), { target: { value: '123456' } }); fireEvent.click(screen.getByRole('button', { name: '验证身份并执行' })); await screen.findByText(/此处是命令处理回执/); expect(screen.queryByText('备份已完成')).not.toBeInTheDocument();
  });
...
describe('S3 operation download error and timeout boundaries', () => {
  ...
  it('allows reauthentication retry without retaining previous password or factor', async () => {
    const value = context(); requestApi.mockRejectedValueOnce(new APIError(403, { code: 'INVALID_SECOND_FACTOR', message: '动态码错误，请重试' })).mockResolvedValueOnce({ reauthToken: 'new-token' }); requestBlob.mockRejectedValue(new APIError(403, { code: 'ARTIFACT_UNAVAILABLE', message: '归档不可用' })); render(<AdminContext.Provider value={value}><OperationDownload operation={{ ...operation, status: 'completed', canDownload: true }} onClose={vi.fn()} /></AdminContext.Provider>);
    fireEvent.change(screen.getByLabelText('下载理由'), { target: { value: '核实完整备份' } }); fireEvent.change(screen.getByLabelText('下载验证密码'), { target: { value: 'old-password' } }); fireEvent.change(screen.getByLabelText('下载动态码或第二因素恢复码'), { target: { value: '000000' } }); fireEvent.click(screen.getByRole('button', { name: '验证并下载归档' })); await screen.findByText('动态码错误，请重试'); expect(value.deny).not.toHaveBeenCalled(); expect(requestBlob).not.toHaveBeenCalled(); expect(screen.getByLabelText('下载验证密码')).toHaveValue(''); expect(screen.getByLabelText('下载动态码或第二因素恢复码')).toHaveValue('');
    fireEvent.change(screen.getByLabelText('下载验证密码'), { target: { value: 'new-password' } }); fireEvent.change(screen.getByLabelText('下载动态码或第二因素恢复码'), { target: { value: '123456' } }); fireEvent.click(screen.getByRole('button', { name: '验证并下载归档' })); await screen.findByText('归档不可用'); expect(requestApi).toHaveBeenCalledTimes(2); expect(requestBlob.mock.calls[0][1]).toMatchObject({ body: { reason: '核实完整备份', reauthToken: 'new-token' }, timeoutMs: 600000 }); expect(value.deny).not.toHaveBeenCalled(); expect(URL.createObjectURL).not.toHaveBeenCalled();
  });
