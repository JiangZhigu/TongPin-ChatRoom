...
export function AdminActionDialog({ request, onClose, onChanged }: { request: ActionRequest; onClose: () => void; onChanged: () => void }) {
  ...
  return <Modal open title={actionLabels[request.action]} onClose={onClose} dismissible={!busy}><div className="admin-action-dialog">
    ...
    {command && <section aria-live="polite"><h3>执行结果：{statusText(command.status)}</h3><p>总计 {command.total} · 成功 {command.succeeded} · 失败 {command.failed} · 待处理 {command.pending}</p>{['queued', 'running'].includes(command.status) && <p>页面可见时每隔至少 2 秒更新进度；关闭窗口不会取消服务器任务。</p>}<ul className="admin-results">{command.items.map((item) => <li key={item.id}><strong>{item.label}</strong><span>{statusText(item.status)}</span>{item.message && <p>{item.message}</p>}{item.code && <small>结果代码：{item.code}</small>}</li>)}</ul><button className="secondary-button" onClick={() => void readCommand()}>刷新操作结果</button>
      ...
      {secret && <div className="admin-secret"><h4>{secret.kind === 'site_invite' || request.action === 'site_invite.create' ? '请安全交付站点邀请码' : '请安全交付给账号本人'}</h4>{secret.kind !== 'site_invite' && request.action !== 'site_invite.create' && <p>账号：{secret.username}</p>}<p><code>{secret.credential}</code></p><p>有效期至 {dateText(secret.expiresAt)}。{secret.kind === 'site_invite' || request.action === 'site_invite.create' ? '受邀者在同频注册页的“站点邀请码”栏填写此内容。' : '本人在同频登录页选择“忘记密码？使用恢复码找回”，使用用户名与此恢复凭据设置新密码。'}</p><button className="secondary-button" onClick={() => setSecret(null)}>隐藏凭据</button></div>}
