...
export function AdminActionDialog({ request, onClose, onChanged }: { request: ActionRequest; onClose: () => void; onChanged: () => void }) {
  ...
  const [password, setPassword] = useState(''); const [factor, setFactor] = useState(''); const [busy, setBusy] = useState(false); const lock = useRef(false);
  ...
  async function execute(event: FormEvent) {
    ...
    try {
      const { reauthToken } = await api<{ reauthToken: string }>('/api/v1/auth/reauth', { method: 'POST', body: { password, secondFactor: factor, action: `admin.execute:${operationId}` }, signal: controller.current?.signal });
      if (!live()) return; setPassword(''); setFactor(''); executionSent = true;
    ...
    finally { if (live()) { setBusy(false); setPassword(''); setFactor(''); } lock.current = false; }
  }
  function resetPreview() { setOperationId(crypto.randomUUID()); setInput(null); setPreview(null); setNeedsPreview(false); setError(''); setPassword(''); setFactor(''); }
  ...
  return <Modal open title={actionLabels[request.action]} onClose={onClose} dismissible={!busy}><div className="admin-action-dialog">
    ...
    {command && <section aria-live="polite"><h3>执行结果：{statusText(command.status)}</h3><p>总计 {command.total} · 成功 {command.succeeded} · 失败 {command.failed} · 待处理 {command.pending}</p>{['queued', 'running'].includes(command.status) && <p>页面可见时每隔至少 2 秒更新进度；关闭窗口不会取消服务器任务。</p>}<ul className="admin-results">{command.items.map((item) => <li key={item.id}><strong>{item.label}</strong><span>{statusText(item.status)}</span>{item.message && <p>{item.message}</p>}{item.code && <small>结果代码：{item.code}</small>}</li>)}</ul><button className="secondary-button" onClick={() => void readCommand()}>刷新操作结果</button>
      {request.action === 'user.password_reset' && command.secretAvailable && !secretAttempted && <><p>恢复凭据仅可由当前会话在生成后 5 分钟内领取一次；凭据有效期以领取后显示为准。</p><button className="primary-button" disabled={busy} onClick={() => void revealSecret()}>显示一次性恢复凭据</button></>}
      {secret && <div className="admin-secret"><h4>请安全交付给账号本人</h4><p>账号：{secret.username}</p><p><code>{secret.credential}</code></p><p>有效期至 {dateText(secret.expiresAt)}。本人在同频登录页选择“忘记密码？使用恢复码找回”，使用用户名与此恢复凭据设置新密码。</p><button className="secondary-button" onClick={() => setSecret(null)}>隐藏凭据</button></div>}
      {secretAttempted && !secret && <p className="warning-note">本次凭据已隐藏、到期或读取结果未知，不能再次读取。请核对交付情况；确需重置时人工重新发起，新凭据会使旧凭据失效。</p>}
    </section>}
