...
export function GroupInviteEntry({ token: initialToken, userId, onClose, onSignIn, onOpenGroup }: { token?: string | null; userId: string | null; onClose: () => void; onSignIn?: () => void; onOpenGroup: (id: string) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    void api<InvitePreview>('/api/v1/group-invites/preview', { inviteToken: token, signal: controller.signal }).then((value) => { if (!controller.signal.aborted) { setPreview(value); if (!uncertainAttempt.current) setResult((current) => value.application || current); } }).catch((cause) => { if (!controller.signal.aborted) setError(cause instanceof Error ? cause.message : '邀请预览加载失败'); }).finally(() => { if (!controller.signal.aborted) setBusy(false); });
  ...
  const resultNotice = canOpen ? '你已是群成员，可以打开群聊。' : result ? `${applicationLabel(result.status)}${result.status === 'approved' && !result.currentMember ? '，当前已不在群中。' : '。'}` : '';
