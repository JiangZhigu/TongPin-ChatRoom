...
describe('M5-UI actual App invitation continuation', () => {
  it('strips token fragment under StrictMode, keeps invitation across login, and never auto-applies', async () => {
    let loggedIn = false; const applications: { url: string; options: RequestInit }[] = []; const previewHeaders: string[] = []; let bootstrapCalls = 0;
    ...
    vi.stubGlobal('fetch', vi.fn((url: string, options: RequestInit = {}) => {
      ...
      if (url.endsWith('/captcha')) return reply({ captchaId: 'captcha-invite', image: 'data:image/png;base64,', expiresAt: Date.now() + 120000 });
      if (url.endsWith('/auth/login')) { loggedIn = true; return reply({ user: actor, csrfToken: 'invite-auth-csrf', expiresAt: Date.now() + 60000 }); }
    ...
    fireEvent.click(screen.getByRole('button', { name: '登录或注册后继续' })); await screen.findByRole('heading', { name: '欢迎回来' }); await waitFor(() => expect(screen.getByRole('button', { name: '刷新图形验证码' })).toBeEnabled());
    fireEvent.change(screen.getByLabelText('用户名'), { target: { value: 'invite_user' } }); fireEvent.change(screen.getByLabelText('密码', { exact: true }), { target: { value: 'ui-test-password-only' } }); fireEvent.change(screen.getByLabelText('图形验证码'), { target: { value: 'ABCDEF' } }); fireEvent.click(screen.getAllByRole('button', { name: '登录' }).at(-1)!);
