...
- **管理员初始化全中文、可重试。** 账号格式不正确、账号已被占用、密码不符合要求或两次密码不一致时，显示中文提示并继续输入。数据目录占用、保存失败或主动取消时，也会给出提示，不再向用户直接输出异常堆栈。
- **管理员使用账号密码登录。** 初始化仅填写账号、密码和确认密码，显示名默认使用账号；新旧管理员登录、接受管理员邀请及敏感操作均无需动态码或绑定验证器。
- **注册后直接进入应用。** 不生成或展示恢复码，也无需保存第二因素恢复码。原有注册图形验证码与连续登录失败后的校验仍按站点规则执行。
- **忘记密码由管理员协助处理。** 管理员核实身份后发放临时重置凭据，用户据此设置新密码；旧版自助恢复码已停用。首位管理员可在服务停止后通过本地维护入口重设密码，详见下方“忘记密码与管理员恢复”。

上述账号流程适用于正式应用。独立演示页使用自己的模拟账号与认证逻辑，其演示凭据及限制见“前端演示”和 [演示说明](demo/README.md)。升级已有站点时，按运维指南保留数据和密钥，并重启服务以载入新版本。
...
| ✨ 消息互动 | 回复、提及、撤回、收藏、搜索定位、举报、通知与消息提示音 |
...
| 👤 账号与设置 | 个人资料、头像、设备会话、隐私与通知设置、管理员协助找回密码、注销冷静期 |
...
| 身份与安全 | Argon2、会话与权限校验 | 密码保护、管理员权限、敏感操作密码验证 |
...
1. **初始化管理员。** 在服务未运行时执行 `manage init-admin`，按中文提示设置账号、密码并确认密码；输入错误会提示重试。显示名默认使用账号，登录后可修改。不需要动态码或恢复码。项目没有默认管理员账号或密码，已有管理员时无需重复初始化。
2. **打开应用。** 启动后访问 [http://127.0.0.1:8765](http://127.0.0.1:8765)，管理后台入口为 [http://127.0.0.1:8765/admin](http://127.0.0.1:8765/admin)。
3. **创建普通账号。** 新站默认允许自主注册，可从登录页切换到“注册”；注册成功后直接进入应用，不需要保存或确认恢复码。管理员可以调整为开放、仅邀请码或关闭注册；已有站点保留原有策略。
...
**普通账号忘记密码：** 联系站点管理员核实身份。管理员在管理后台的“用户管理”中打开该用户详情，选择“人工重置密码”，填写原因并验证管理员自己的当前密码。系统生成一份 **1 小时内有效、仅能使用一次** 的重置凭据，由管理员交给账号本人；用户在登录页点击“忘记密码？联系管理员”，填写账号、管理员提供的重置凭据和新密码。重置操作会使旧设备会话失效，完成后使用新密码重新登录。临时凭据仅在需要找回密码时由管理员发放，注册时无需领取或保存。

**首位管理员忘记密码：** 由有部署主机访问权限的维护人员先停止服务，再在原项目目录执行：

```powershell
# Windows PowerShell
.\tongpin.cmd manage recover-admin
```

```sh
# Linux / macOS
sh ./tongpin.sh manage recover-admin
```

按中文提示填写已有管理员账号、恢复原因、确认账号和新密码。操作会记录审计并使该管理员的旧会话失效，无需动态码或恢复码。请沿用原有数据目录与配置；详细管理规则见 [超级管理员指南](docs/ADMIN_GUIDE.zh-CN.md)。
